...
Additional D41/D42 tests cover Claude Code CLI override-first discovery and PATH fallback, argv construction with inline shared schemas and no session persistence, permission-mode mapping, result-vs-structured-output separation without JSON transcript leakage, direct reported-cost accounting, permission-denial diagnostics, fake executable envelope parsing through `execa`, shared strict schema reuse, prompt parity, and Gemini+Claude, Claude+MiniMax, and Codex+Claude mixed-engine combinations.
