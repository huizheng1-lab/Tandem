...
- D38-1/D38-2: done - Codex-backed workers now receive the same host-platform, project-instruction, blind-media, and exact verification command reporting instructions as AI-SDK workers, with project instructions threaded from `createLiveAgents`; investigated the live-observed stray `.git`/`.agents` directories and did not add cleanup because the current repo has a valid `.git`, no `.agents`, and no confirmed safe attribution beyond Codex CLI workspace-write behavior.
- D39-1/D39-2: done - Codex `agent_message` JSONL events are no longer forwarded to live transcript text because all Tandem Codex invocations use `--output-schema`; parsed artifacts and clean answer summaries still surface through normal machine/done events while tool activity remains unchanged.
- D40-1: done - the desktop sidebar now scrolls vertically as a whole when accumulated sessions, goals, and schedules exceed the window height while continuing to clip horizontal overflow.
- D41-1/D41-6: done - added `claude-code/cli` as a first-class CLI-backed provider with configurable binary discovery, shared Codex schema reuse, single-envelope Claude Code exec parsing, `result`-only prose forwarding, `structured_output` artifact validation, permission-denial diagnostics, direct `total_cost_usd` ledger accounting, leader/worker/review/takeover integration, desktop/TUI/CLI model availability labels, and mixed API/Codex/Claude engine coverage. Read-only leader phases use Claude Code `plan` mode plus a `Read,Grep,Glob` tool allowlist per the verified handoff facts; write phases map `yolo` to `bypassPermissions`, `auto-edit` to `acceptEdits`, and `ask` to the existing round-level confirmation flow followed by `bypassPermissions`.
- D42-1/D42-2: done - Claude Code CLI execution now uses `execa` with `cancelSignal` instead of raw `spawn` plus Windows `shell: true`, so large prompts and inline JSON schemas are passed as real arguments to standard npm `.cmd` shims; Claude CLI discovery now honors `claudeCliPath`/`CLAUDE_CLI_PATH` before PATH to match Codex override semantics. Codex CLI exec was left on its existing raw-spawn path because the verified local Codex install resolves to a direct `.exe`, so it does not hit the `.cmd` shell-concatenation failure fixed here; a future Codex `.cmd` distribution would be a reasonable follow-up migration.
- D43-1: done - Claude Code CLI calls now pass Tandem role/rule/platform/project-instruction text through `--system-prompt` and keep only task-specific request/artifact context in the positional `-p` user prompt for leader planning, review, takeover, and worker build. A cheap live `haiku` check showed both `--system-prompt` and `--append-system-prompt` produced immediate structured envelopes for a direct math request; chose `--system-prompt` because it is leaner and avoids blending Tandem's role contract with Claude Code's default persona.
...
- `npm test`: passed. 23 test files, 150 tests; 1 live-smoke test skipped unless `RUN_LIVE=1`.
...
Additional D38 tests cover Codex worker prompt parity for project instructions, host-platform guidance, blind-media handling, exact BuildPlan verification command echoing, and worker context inclusion.
Additional D39 tests cover Codex JSONL `agent_message` events not calling live text callbacks while command activity and token usage still flow.
D40 is a CSS overflow fix; automated TypeScript and unit tests pass, and the acceptance sidebar-scroll probe remains a reviewer live-window check.
Additional D41-D43 tests cover Claude Code CLI override-first discovery and PATH fallback, argv construction with inline shared schemas, no session persistence, and `--system-prompt`, permission-mode mapping, result-vs-structured-output separation without JSON transcript leakage, direct reported-cost accounting, permission-denial diagnostics, fake executable envelope parsing through `execa`, shared strict schema reuse, prompt parity, leader/worker system-vs-user prompt separation, and Gemini+Claude, Claude+MiniMax, and Codex+Claude mixed-engine combinations.
