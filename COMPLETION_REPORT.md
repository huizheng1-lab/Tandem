...
- D41-1/D41-6: done - added `claude-code/cli` as a first-class CLI-backed provider with PATH-first/override-fallback discovery, shared Codex schema reuse, single-envelope Claude Code exec parsing, `result`-only prose forwarding, `structured_output` artifact validation, permission-denial diagnostics, direct `total_cost_usd` ledger accounting, leader/worker/review/takeover integration, desktop/TUI/CLI model availability labels, and mixed API/Codex/Claude engine coverage. Read-only leader phases use Claude Code `plan` mode plus a `Read,Grep,Glob` tool allowlist per the verified handoff facts; write phases map `yolo` to `bypassPermissions`, `auto-edit` to `acceptEdits`, and `ask` to the existing round-level confirmation flow followed by `bypassPermissions`.
...
Additional D41 tests cover Claude Code CLI PATH-first discovery and override fallback, argv construction with inline shared schemas and no session persistence, permission-mode mapping, result-vs-structured-output separation without JSON transcript leakage, direct reported-cost accounting, permission-denial diagnostics, fake executable envelope parsing, shared strict schema reuse, prompt parity, and Gemini+Claude, Claude+MiniMax, and Codex+Claude mixed-engine combinations.
