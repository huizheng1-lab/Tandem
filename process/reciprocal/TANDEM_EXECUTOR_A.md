...
- `A_UPGRADE_PENDING`: stop for the human A-runtime promotion gate. You may report the `PrepareAUpgrade -DryRun` command.
- `WAIT` or `PAUSED`: answer with the relay status and stop.

For implementation, claim a shared-direction item with `scripts/reciprocal-direction.ps1 -Action Start -Id <id> -Role A`, keep `.tandem/reciprocal-checkpoint.md` current, implement a narrow one-commit candidate, and run focused checks plus `npm run typecheck` and `git diff --check`. Keep `authoritative-only: npm test` in the plan verification list so Tandem's authoritative runner executes the full suite outside the sandbox.
