...
The reciprocal launcher gives Codex's sandbox write access to the shared control directory and the admin repository's common `.git` directory. Linked worktrees place Git's index locks, object writes, branch refs, reflogs, packed-ref rewrites, and relay refs in that common Git directory, not in the worktree checkout. This permission is for Git plumbing and relay commands only; Tandem's protected-root guard still blocks source edits to the admin repository, peer worktree, runtimes, and state stores outside the selected target.
