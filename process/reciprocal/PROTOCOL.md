...
3. Interpret the result:
   - `CLAIMED`: the relay assigned this turn to you. Tandem's app layer pre-fast-forwards a clean reciprocal worktree from its peer before the sandboxed invocation starts, because linked-worktree Git metadata locks are not writable from Codex.
   - `VALIDATE`: the other executor produced a candidate. Do not start a full agentic peer Tandem session. Run the role's `Validate` relay command in the validating worktree; it directly executes `npm run typecheck`, `npm test`, and a candidate `git diff --check`, captures the real output, then invokes exactly one read-only leader review through `AgentFns.review(plan, report, diff)`. If mechanical checks fail, validation rejects the candidate and creates the normal auditable rollback. If the leader verdict is `approve`, validation accepts the candidate through the relay. If the leader verdict is `revise` or `takeover`, validation rejects the candidate and rolls it back with the review feedback. If a rollback candidate fails validation, pause for human inspection instead of reverting the revert. When an accepted fully autonomous epic exposes `autonomousContinuation.available=true`, the accepting executor already owns the next `working` turn and may immediately start that one recorded next step in the same invocation; after producing that step candidate, stop normally.
...
4. Do not begin an improvement until the relay is in `working` phase. Read `.tandem/shared-control/SHARED_DIRECTION.md` with `scripts/reciprocal-direction.ps1 -Action Show`. Select the highest-priority claimable human wishlist item: a normal `QUEUED` item, an epic `QUEUED` for its plan-only turn, or an approved epic with a recorded next step and no current owner. An epic plan candidate is not claimable until it is `PLAN_APPROVED`, whether approval is human or automatic. The only exception to the usual one-lifecycle stop is the explicit autonomous continuation returned by `Accept`/`Validate`: for `autonomousContinuation.available=true`, run the returned `Start` command for that same wishlist item and produce exactly one next-step candidate before stopping. Never chain a second continuation. Until a human explicitly removes this restriction after a few reviewed batches, do not self-select `[AUTO]` improvements; if no human item is claimable, use `Pause` with reason `no queued human item`.
...
8. Inside a sandboxed producing turn, run focused tests for every changed behavior plus, at minimum:
   - `npm run typecheck`
   - `git diff --check`
   Never weaken, skip, or suppress a test that can run in the sandbox. Every implementation plan must still list the full suite in its verification array as `authoritative-only: npm test`. Tandem's authoritative verification runner strips the prefix and executes that full suite outside the producing Codex sandbox after the worker returns, while the worker reports the prefixed command verbatim with the authoritative-only skipped marker. The opposite executor's `Validate` command repeats the full suite mechanically before stable advances. On Windows, do not repeatedly retry a full-suite command inside Codex when workspace-write blocks Vitest configuration loading or child-process cleanup; only that redundant in-sandbox invocation is relocated. The two authoritative full-suite gates remain mandatory, and a failure at either gate prevents acceptance.
...
2. The opposite executor validates that plan candidate through the normal relay. For a plan-gated epic, implementation remains blocked until a human uses the dashboard plan gate. For a fully autonomous epic, relay `Accept` runs `reciprocal-direction.ps1 -Action AutoApprovePlan -Id <id> -Commit <accepted-stable>`; this records `approval=auto`, writes the plan auto-approval audit entry, changes the board to `PLAN_APPROVED` without a human click, and returns an `autonomousContinuation` payload for the next step when one exists.
...
4. After peer validation of a non-final fully autonomous step, relay `Accept` records `AcceptStep -Commit <stable>` and returns an `autonomousContinuation` payload for the next step. Plan-gated epics do not auto-continue. The final accepted step uses `Complete`, moves the epic from `CANDIDATE` to `DONE`, and returns no continuation.
