...
- [x] Step 2: replace the renderer's silent-stall inference with the orchestrator's run-health state, so the activity strip and session log show a single honest "healthy / quiet Ns / likely stalled" indicator instead of a stale "thinking..." pulse.
